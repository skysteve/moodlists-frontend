...
function init(componentList: any) {
  ...
  window.addEventListener(EventTypes.userData, (event: UserDataLoadedEvent) => {
    elContainer.innerHTML =
      "<section-seed-list></section-seed-list><section-knobs-panel collapsed></section-knobs-panel><section-recommendations-list collapsed></section-recommendations-list>";
  });

  window.addEventListener(
    EventTypes.userSigninRequired,
    (event: UserSigninRequiredEvent) => {
      elContainer.innerHTML = "<h1>Welcome, please sign in</h1>";
    }
  );

  window.addEventListener(EventTypes.allSeedsSelectedEvent, () => {
    const elSeedList = elContainer.querySelector(
      "section-seed-list"
    ) as SectionElement;
    const elKnobsPanel = elContainer.querySelector(
      "section-knobs-panel"
    ) as SectionElement;
