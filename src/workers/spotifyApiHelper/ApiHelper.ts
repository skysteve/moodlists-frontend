...
const AUTH_URL = "https://iskpng28a8.execute-api.eu-west-1.amazonaws.com/Prod"; //'http://localhost:3000'; //'http://localhost:3847'; //
...
export class ApiHelper {
          ...
          .map(k => {
            ...
            if (k === "duration") {
              return `min_duration_ms=${(
                knobs[k].min * 1000
              ).toString()}&max_duration_ms=${(
                knobs[k].max * 1000
              ).toString()}`;
            }

            return [
              `min_${encodeURIComponent(k)}=${encodeURIComponent(
                (knobs[k].min / 100).toString()
              )}`,
              `max_${encodeURIComponent(k)}=${encodeURIComponent(
                (knobs[k].max / 100).toString()
              )}`
            ].join("&");
          })
          .join("&");
    const queryString = `seed_artists=${seedArtists.join(
      ","
    )}&market=US&${knobsToQS}&limit=${limit}`;
  ...
  public async createPlaylist(playlistData: IPlaylistData) {
    ...
    const createdPlaylist = await this.makeRequest(
      `me/playlists`,
      REQUEST_TYPE.POST,
      postBody
    );
  ...
  private async makeRequest(
    path: string,
    type: REQUEST_TYPE = REQUEST_TYPE.GET,
    postBody?: any
  ) {
  ...
  private async refreshUserToken() {
    ...
    const res = await fetch(
      `${AUTH_URL}/refresh-token?refresh_token=${this.refreshToken}`
    );
